...
import com.example.moviesbasic.screen.popular.LoadingScreen
...
@Composable
fun DetailScreen(imageData: ImageDto, detailData: DetailDto, movie: Result?, castData: CastDto) {
            ...
            Scaffold(
                ...
                Box(
                    ...
                    HorizontalPager(state = tabPagerState , beyondBoundsPageCount = 2) {index->
                        when (index) {
                            0 -> {
                                DetailTab(detailData = detailData, movie = movie, castData = castData)
                            }
                            1 -> {
...
private fun DetailScreenPreview() {
    ...
    val movieAppUiState = movieAppViewModel.popularMovieState
    val data : PopularMovieDto? = when(movieAppUiState){
        is MovieAppUiState.PopularMovie -> movieAppUiState.data
        else -> null
    }
    val result = data?.results?.find {
        it?.id == 693134
    }
    movieAppViewModel.getPopularMovieList()
    val detailScreenViewModel:DetailScreenViewModel = viewModel()
    detailScreenViewModel.getMovieDetails("693134")
    detailScreenViewModel.getMovieImage("693134")
    ...
    detailScreenViewModel.getMovieCast("693134")
    when (movieImageUiState) {
        is DetailScreenUiState.Loading -> LoadingScreen(detailScreenViewModel, "693134")
